...
def safe_sleep(seconds: float, jitter: float = 0.3):
    """Sleeps with random humanized jitter to prevent Discord automated bot detection patterns."""
    actual_delay = max(0.1, seconds + random.uniform(0.05, jitter))
...
def fetch_user_dms(headers: Dict[str, str]) -> List[Dict[str, Any]]:
    """Fetches user's active DM channels with rate-limit handling."""
    try:
        ...
        if res.status_code == 200:
            ...
            retry = float(res.json().get("retry_after", 2.0))
            log_warn(f"Rate limited while fetching DMs. Waiting {retry + 1.5:.2f}s...")
            safe_sleep(retry + 1.5)
            return fetch_user_dms(headers)
        else:
...
def fetch_user_relationships(headers: Dict[str, str]) -> List[Dict[str, Any]]:
    """Fetches user's friends and relationships with rate-limit protection."""
    try:
        ...
        if res.status_code == 200:
            ...
            retry = float(res.json().get("retry_after", 2.0))
            log_warn(f"Rate limited on relationships. Waiting {retry + 2.5:.2f}s...")
            safe_sleep(retry + 2.5)
            return fetch_user_relationships(headers)
        else:
            return []
    except Exception as e:
        ...
        if relationships:
            ...
            for idx, rel in enumerate(relationships, 1):
                ...
                if u_id and u_id not in known_recipients:
                    ...
                    # Humanized pause before checking/unhiding DM
                    safe_sleep(2.5, 2.0)
...
def get_or_create_dm_by_user_id(headers: Dict[str, str], target_user_id: str) -> Optional[Dict[str, Any]]:
    """Opens or retrieves an existing DM channel with a target user by User ID with rate-limit safety."""
    ...
    try:
        ...
        if res.status_code in (200, 201):
            ...
            retry = float(res.json().get("retry_after", 2.0))
            log_warn(f"Rate limited while opening DM. Waiting {retry + 2.0:.2f}s...")
            safe_sleep(retry + 2.0)
            return get_or_create_dm_by_user_id(headers, target_user_id)
...
def fetch_guild_channels(headers: Dict[str, str], guild_id: str) -> List[Dict[str, Any]]:
    """Fetches text channels in a guild with rate-limit protection."""
    try:
        ...
        if res.status_code == 200:
            ...
            retry = float(res.json().get("retry_after", 2.0))
            log_warn(f"Rate limited while fetching channels. Retrying in {retry + 1.5:.2f}s...")
            safe_sleep(retry + 1.5)
            return fetch_guild_channels(headers, guild_id)
        else:
    ...
    while True:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                log_warn(f"Search API Rate limited. Waiting {retry + 2.0:.2f} seconds...")
                safe_sleep(retry + 2.0)
            elif res.status_code == 202:
                log_warn("Discord is indexing messages for this server. Retrying in 5s...")
                safe_sleep(5.0)
            else:
    ...
    while True:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                retry = float(res.json().get("retry_after", 2.0))
                log_warn(f"\nRate limited during message scan. Waiting {retry + 1.5:.2f}s...")
                safe_sleep(retry + 1.5)
            else:
...
def delete_message(headers: Dict[str, str], channel_id: str, message_id: str) -> tuple[bool, bool]:
    """
    Sends DELETE request for a single message and handles rate limits safely.
    ...
    while True:
        try:
            ...
            if res.status_code == 204:
                ...
                log_warn(f"Rate limited by Discord. Waiting {retry + 1.5:.2f}s for safe reset...")
                safe_sleep(retry + 1.5)
...
def delete_message_batch(
    ...
    """
    Deletes a list of messages one by one with rate-limit handling, humanized jitter, and anti-ban cooldowns.
    ...
    deleted_count = 0
    failed_count = 0
    current_delay = max(1.4, base_delay)
    ...
    try:
        for idx, msg in enumerate(messages_to_delete, 1):
            ...
            if dry_run:
                ...
                if hit_rl:
                    consecutive_smooth = 0
                    current_delay = min(current_delay + 0.4, 4.0)
                    log_info(f"Adaptive delay increased to {current_delay:.2f}s to shield account.")
                else:
                    consecutive_smooth += 1
                    if consecutive_smooth >= 12 and current_delay > base_delay:
                        current_delay = max(current_delay - 0.1, base_delay)
                        consecutive_smooth = 0
                
                safe_sleep(current_delay, 0.4)

                # Periodic Anti-Ban Cool-Down Pause (every 20 messages)
                if deleted_count > 0 and deleted_count % 20 == 0 and idx < total_msgs:
                    log_info(f"{Fore.YELLOW}[Anti-Ban Guard]{Style.RESET_ALL} Pausing 4.0s cooldown to protect account from automated detection...")
                    safe_sleep(3.5, 0.8)
...
def process_bulk_user_queue(
    ...
    for idx, uid in enumerate(user_ids, 1):
        ...
        if idx < total_users:
            log_info("Resting 2.5s before moving to next user in queue...")
            safe_sleep(2.0, 0.6)
...
def process_all_dms_queue(
    ...
    for idx, dm in enumerate(dm_list, 1):
        ...
        if idx < total_chats:
            log_info("Resting 3.5s before moving to next DM chat in queue...")
            safe_sleep(3.0, 1.0)
